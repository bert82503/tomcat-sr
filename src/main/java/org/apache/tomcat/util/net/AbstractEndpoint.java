...
public abstract class AbstractEndpoint<S> {
	...
	public static interface Handler {
		...
		/**
		 * Different types of socket states to react upon. ("套接字"状态)
		 */
		public enum SocketState {
			...
			OPEN, // 打开中
			CLOSED, // 已关闭
			LONG, ASYNC_END, SENDFILE, UPGRADING_TOMCAT, UPGRADING, UPGRADED
		}
	...
	/**
	 * Running state of the endpoint. (端点的运行中状态)
	 */
	protected volatile boolean running = false;

	/**
	 * Will be set to true whenever the endpoint is paused. (当端点暂停时，会将其设置为 true)
	 */
	protected volatile boolean paused = false;

	/**
	 * Are we using an internal executor (是否使用内部执行器，默认是不使用(false) )<br>
	 */
	protected volatile boolean internalExecutor = false;

	/**
	 * counter for nr of connections handled by an endpoint (请求连接计数器，用于限制)
	 */
	private volatile LimitLatch connectionLimitLatch = null;

	/**
	 * Socket properties (套接字属性集)
	 */
	 ...
	 * endpoint is stopped in milliseconds. Defaults to 5000 (5 seconds). <br>
	 ...
	 * accept, but not process, one further connection. This additional
	 * connection be blocked until the number of connections being processed
	 * falls below maxConnections at which point the server will start accepting
	 * and processing new connections again. Note that once the limit has been
	 * reached, the operating system may still accept connections based on the
	 * acceptCount setting. The default is 10000. <br>
	...
	/**
	 * The minimum number of threads always kept running. The default is 10. <br>
	...
	/**
	 * Maximum amount of worker threads. <br>
	 * 工作者线程的最大数量（默认是 200）
	 */
